...
// 1️⃣ - جلب بيانات المستخدم (لصورة البروفايل واسم المستخدم)
$stmt = $pdo->prepare("SELECT username, profile_img FROM users WHERE id = ?");
$stmt->execute([$_SESSION['user_id']]);
$user = $stmt->fetch(PDO::FETCH_ASSOC);

// 3️⃣ - جلب بوستات المستخدم مع اسم المستخدم من خلال JOIN
$stmt = $pdo->prepare("
...
// 2️⃣ - التعامل مع رفع صورة البروفايل
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['profile_image'])) {
    $imageName = basename($_FILES['profile_image']['name']);
    $imageTmp = $_FILES['profile_image']['tmp_name'];
    $imagePath = 'upload/' . time() . '_' . $imageName;
...
// 4️⃣ - التعامل مع اللايك والديسلايك والتعليقات
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // لا تتداخل مع رفع الصورة
    if (isset($_POST['post_id'])) {
        ...
        } elseif (isset($_POST['comment_text']) && trim($_POST['comment_text']) !== '') {
    ...
    <div class="profile">
        ...
        <form method="POST" enctype="multipart/form-data">
            <a href="index.php" class="home-link">
                <i class="fas fa-home" style="font-size: 30px; color: #4CAF50;"></i>
            </a>
            <br><br>
            <input type="file" name="profile_image" id="profile_image" hidden>
            <label for="profile_image" class="button" style="background-color: teal; color: white; border: none; padding: 8px 16px; border-radius: 10px;">Choose IMG</label>
            <button type="submit" class="button" style="background-color: teal; color: white; border: none; padding: 8px 16px; border-radius: 10px;">UP</button>
        </form>
    </div>
        ...
        <p style="margin-right: 200px;">Welcome, <?= htmlspecialchars($_SESSION['username']) ?>!<br>
            <a href="profile.php">
                <button style="text-decoration: none; color: teal;background-color:aqua">
                    <i class="fas fa-user-circle" style="font-size: 25px; margin-right: 5px;"></i></button>
            </a><br><br>
